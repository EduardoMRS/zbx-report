...
function handleConfigSave($method, $dataReceiver)
{
    ...
    $new_config = json_decode($dataReceiver['config'], true) ?? null;
    ...
    // Mescla os arrays
    $config_save = array_merge($config_default, $new_config);
    ...
    if (!empty($_FILES['pdf_template'])) {
        $target = __DIR__ . "/../../consumption-report/timbrado.pdf";
        move_uploaded_file($_FILES['pdf_template']['tmp_name'], $target);
    }

    // Processar upload do certificado
    if (!empty($_FILES['digital_certificate'])) {
        $target = __DIR__ . "/../../signature/cert.p12";
        move_uploaded_file($_FILES['digital_certificate']['tmp_name'], $target);
    }
